...
const DICT = {
    'ua': {
        ...
        ui_burn_next: "ЗНИЩИТИ Й ДАЛІ", ui_burn_sms: "ЗНИЩИТИ SMS"
    },
    'en': {
        ...
        ui_burn_next: "BURN & NEXT", ui_burn_sms: "BURN SMS"
    }
};
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    window.addEventListener('popstate', (e) => {
        if (isBusy || pc || !document.getElementById("activeCallUI").classList.contains("hidden") || !document.getElementById("incomingUI").classList.contains("hidden")) { history.pushState(null, null, location.href); if (remoteNum) sendWS({ type: "hangup", to: remoteNum }); resetToDialer(); } 
            ...
            document.getElementById("mainApp").classList.add("hidden"); document.getElementById("loginScreen").classList.remove("hidden"); isSystemInitialized = false; 
            ...
            // Видаляємо кнопку блокування, якщо вийшли
            const lb = document.getElementById("lockBtn"); if (lb) lb.remove();
        }
    });

    bind("startBtn", async () => { 
        const v = document.getElementById("passInput").value; if (v.length < 4) return; document.getElementById("loginScreen").classList.add("hidden"); document.getElementById("mainApp").classList.remove("hidden"); history.pushState(null, null, location.href); await deriveSessionKey(v); initWS(); 
